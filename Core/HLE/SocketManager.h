...
struct VirtualPacket {
	...
	std::tuple<int, std::unique_ptr<char[]>> Pack(VirtualSockAddr dest) {
		...
		//   p2p (TCP and UDP):          [VPORT_HEADER][VPKT ext header][seq_id][payload]
...
struct InetSocket {
	...
	virtual int Send_Reliable(const char* buf, int len, int flags, const SceNetInetSockaddr* to, int tolen);
	...
	virtual bool Process_Reliable(VirtualPacket&& vpkt, VirtualSockAddr dest);
	// Process P2P as UDP
	...
	virtual bool Process_Unreliable(VirtualPacket&& vpkt, VirtualSockAddr dest);
