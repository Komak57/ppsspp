...
void __NpInit() {
	...
	user_id.store(0); // Zero indicates no-linked-user - will be update by the NPServer login
}
void __NpShutdown() {
...
std::string* NpGetLogin() {
	...
	creds[0] = g_Config.sInfraNpId;
	creds[1] = g_Config.sInfraPassword;
	creds[2] = g_Config.sInfraToken;
	return creds;
}
...
int sceNpAuthGetTicket(u32 requestId, u32 bufferAddr, u32 length) {
	...
	if (g_Config.sInfraNpId.empty() || g_Config.sInfraNpId != SanitizeString(g_Config.sInfraNpId, StringRestriction::AlphaNumUnderscore, 3, 16)) {
	...
	ofs += writeTicketStringParam(buf + ofs, PARAM_TYPE_STRING, g_Config.sInfraNpId.c_str(), 32); // username (pre-cut to 16 chars)
...
void np2ValidateLoopMemory() {
	...
	if (!npMatching2ThreadHackAddr || (npMatching2ThreadHackAddr && strcmp("SceNpMatching2", kernelMemory.GetBlockTag(npMatching2ThreadHackAddr)) != 0)) {
		u32 blockSize = sizeof(npMatching2ThreadCode);
		npMatching2ThreadHackAddr = kernelMemory.Alloc(blockSize, false, "SceNpMatching2");
		if (npMatching2ThreadHackAddr) Memory::Memcpy(npMatching2ThreadHackAddr, npMatching2ThreadCode, sizeof(npMatching2ThreadCode));
	}
}
...
const HLEFunction sceNpCommerce2[] = {
	...
	{0x05E7AFBC, nullptr,                            "sceNpCommerce2GetGameProductInfoFromContentInfo",			'?', ""   },
	{0X0E9956E3, &WrapI_V<sceNpCommerce2Init>,       "sceNpCommerce2Init",										'i', ""   },
	{0X1888A9FE, nullptr,                            "sceNpCommerce2DestroyReq",								'?', ""   },
	{0x1C85ED88, nullptr,                            "sceNpCommerce2GetPrice",									'?', ""   },
	...
	{0x490210E0, nullptr,                            "sceNpCommerce2DestroyGetProductInfoResult",				'?', ""   },
	{0X4ECD4503, nullptr,                            "sceNpCommerce2CreateSessionCreateReq",					'?', ""   },
	{0x54FE8871, nullptr,                            "sceNpCommerce2GetCategoryInfo",							'?', ""   },
	{0x57C8BD4E, nullptr,                            "sceNpCommerce2GetCategoryInfoFromContentInfo",			'?', ""   },
	{0X590A3229, nullptr,                            "sceNpCommerce2GetSessionInfo",							'?', ""   },
	{0x5D8C2D99, nullptr,                            "sceNpCommerce2GetCategoryContentsCreateReq",				'?', ""   },
	{0x6592FE83, nullptr,                            "sceNpCommerce2GetContentRatingDescriptor",				'?', ""   },
	{0x6A2AE572, nullptr,                            "sceNpCommerce2GetContentRatingInfoFromGameProductInfo",	'?', ""   },
	{0X6F1FE37F, nullptr,                            "sceNpCommerce2CreateCtx",									'?', ""   },
	{0x80A7AFDB, nullptr,                            "sceNpCommerce2GetCategoryContentsGetResult",				'?', ""   },
	{0XA5A34EA4, &WrapI_V<sceNpCommerce2Term>,       "sceNpCommerce2Term",										'i', ""   },
	{0XAA4A1E3D, nullptr,                            "sceNpCommerce2GetProductInfoGetResult",					'?', ""   },
	{0xB0D7AA90, nullptr,                            "sceNpCommerce2GetContentInfo",							'?', ""   },
	{0XBC61FFC8, nullptr,                            "sceNpCommerce2CreateSessionGetResult",					'?', ""   },
	{0xC5505A19, nullptr,                            "sceNpCommerce2GetContentRatingInfoFromCategoryInfo",		'?', ""   },
	{0XC7F32242, nullptr,                            "sceNpCommerce2AbortReq",									'?', ""   },
	{0xCEAB1829, nullptr,                            "sceNpCommerce2InitGetCategoryContentsResult",				'?', ""   },
	{0xDD6758FA, nullptr,                            "sceNpCommerce2GetCategoryContentsStart",					'?', ""   },
	{0xED85ACCE, nullptr,                            "sceNpCommerce2DestroyGetCategoryContentsResult",			'?', ""   },
