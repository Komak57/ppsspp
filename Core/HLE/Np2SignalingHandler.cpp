...
void SignalingCommandToString(std::string& out, u64 arg)
{
	switch (arg)
	{
	case signal_ping: out = "PING"; return;
	case signal_pong: out = "PONG"; return;
	case signal_connect: out = "CONNECT"; return;
	case signal_connect_ack: out = "CONNECT_ACK"; return;
	case signal_confirm: out = "CONFIRM"; return;
	case signal_finished: out = "FINISHED"; return;
	case signal_finished_ack: out = "FINISHED_ACK"; return;
	case signal_info: out = "INFO"; return;
	}
	out = "UNKNOWN";
	return;
}
...
void signaling_handler::signal_sig_callback(u32 conn_id, s32 event, s32 error_code)
{
	for (const auto& ctx_id : sig_ctx_lst)
	{
		...
		if (ctx->handler)
		{
			...
			NOTICE_LOG(Log::sceNet, "Called sig CB: 0x%x (conn_id: %d)", event, conn_id);
		}
	}
...
void signaling_handler::signal_ext_sig_callback(u32 conn_id, s32 event, s32 error_code) const
{
	for (const auto ctx_id : sig_ctx_lst)
	{
		...
		if (ctx->ext_handler)
		{
			...
			NOTICE_LOG(Log::sceNet, "Called EXT sig CB: 0x%x (conn_id: %d)", event, conn_id);
		}
	}
}
...
{
	if (sp->signature != (u32_be)SIGNALING_SIGNATURE)
	{
		ERROR_LOG(Log::sceNet, "Received a signaling packet with an invalid signature");
		return false;
	}

	if (sp->version != SIGNALING_VERSION)
	{
		ERROR_LOG(Log::sceNet, "Invalid version in signaling packet: %d, expected: %d", sp->version, SIGNALING_VERSION);

		if (sp->version > SIGNALING_VERSION)
			ERROR_LOG(Log::sceNet, "You are most likely using an outdated version of RPCS3");
		else
			ERROR_LOG(Log::sceNet, "The other user is most likely using an outdated version of RPCS3");
	...
	{
		ERROR_LOG(Log::sceNet, "Invalid npid in signaling packet");
		return false;
	}
...
void signaling_handler::process_incoming_messages()
{
	...
	for (const auto& msg : msgs)
	{
		...
		{
			ERROR_LOG(Log::sceNet, "Received an invalid signaling packet");
			continue;
		}
...

		{
			...
			INFO_LOG(Log::sceNet, "SP %s from %s:%d(npid: %s)", sp->command, ip_str, op_port, npid);
		}
		...
		const auto update_rtt = [&](u64 rtt_timestamp)
		{
			...
			std::size_t num_rtts = min(static_cast<std::size_t>(6), si->rtt_counters);
			u64 sum = 0;
		...
		case signal_connect_ack:
			...
			update_si_status(si, SCE_NP_SIGNALING_CONN_STATUS_ACTIVE, 0);
			break;
		case signal_confirm:
		...
		case signal_finished_ack:
			...
			break;
		default: ERROR_LOG(Log::sceNet, "Invalid signaling command received"); continue;
		}
...
void signaling_handler::update_si_addr(std::shared_ptr<signaling_info>& si, u32 new_addr, u16 new_port)
{
	...
	if (si->addr != new_addr || si->port != new_port)
	{
		if (sign_log.trace)
		{
			...
			INFO_LOG(Log::sceNet, "Updated Address from %s:%d to %s:%d", ip_str_old, si->port, ip_str_new, new_port);
		}
	...
	{
		{
			...
			INFO_LOG(Log::sceNet, "Updated Mapped Address from %s:%d to %s:%d", ip_str_old, si->mapped_port, ip_str_new, new_port);
		}
...
void signaling_handler::update_si_status(std::shared_ptr<signaling_info>& si, s32 new_status, s32 error_code)
{
	...
	if (si->conn_status == SCE_NP_SIGNALING_CONN_STATUS_PENDING && new_status == SCE_NP_SIGNALING_CONN_STATUS_ACTIVE)
	{
		...
		if (si->op_activated)
			signal_ext_sig_callback(si->conn_id, SCE_NP_SIGNALING_EVENT_EXT_MUTUAL_ACTIVATED, SCE_NP_MATCHING2_OKAY);
	}
...
void signaling_handler::update_ext_si_status(std::shared_ptr<signaling_info>& si, bool op_activated)
{
	...
	if (op_activated && !si->op_activated)
	{
		...
		if (si->conn_status != SCE_NP_SIGNALING_CONN_STATUS_ACTIVE)
			signal_ext_sig_callback(si->conn_id, SCE_NP_SIGNALING_EVENT_EXT_PEER_ACTIVATED, SCE_NP_MATCHING2_OKAY);
		else
			signal_ext_sig_callback(si->conn_id, SCE_NP_SIGNALING_EVENT_EXT_MUTUAL_ACTIVATED, SCE_NP_MATCHING2_OKAY);
	}
	else if (!op_activated && si->op_activated)
	{
		...
		signal_ext_sig_callback(si->conn_id, SCE_NP_SIGNALING_EVENT_EXT_PEER_DEACTIVATED, SCE_NP_MATCHING2_OKAY);
	}
}
...
void signaling_handler::send_signaling_packet(signaling_packet& sp, u32 addr, u16 port) const
{
	...
	INFO_LOG(Log::sceNet, "Sending %s packet to %s:%d", sp.command, ip_str, port);
	...
	if (np::is_ipv6_supported() && np::ip_address_translator::is_ipv6(dest.sin_addr.s_addr))
	{
		...
		if (!send_packet_from_p2p_port_ipv6(packet, addr6))
			ERROR_LOG(Log::sceNet, "Failed to send signaling packet to %s:%d", ip_str, port);
	}
...
std::shared_ptr<signaling_info> signaling_handler::get_signaling_ptr(const signaling_packet* sp)
{
	...
	{
		ERROR_LOG(Log::sceNet, "Discrepancy in signaling data");
		return nullptr;
	}
...
u32 signaling_handler::init_sig1(const SceNpId& npid)
{
	...
	{
		INFO_LOG(Log::sceNet, "Creating new sig1 connection and requesting infos from RPCN");
...
u32 signaling_handler::init_sig2(const SceNpId& npid, u64 room_id, u16 member_id)
{
	...
	if (si->conn_status == SCE_NP_SIGNALING_CONN_STATUS_ACTIVE)
		signal_sig2_callback(si->room_id, si->member_id, SCE_NP_MATCHING2_SIGNALING_EVENT_Established, SCE_NP_MATCHING2_OKAY);
	else
