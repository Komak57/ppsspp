...
namespace np
{
	...
	void RoomMemberDataInternalUpdateInfo_to_SceNpMatching2RoomMemberDataInternalUpdateInfo(BlockAllocator& edata, const RoomMemberDataInternalUpdateInfo* update_info, SceNpMatching2RoomMemberDataInternalUpdateInfo* sce_update_info, bool include_onlinename, bool include_avatarurl)
	{
		...
		if (update_info->newRoomMemberBinAttrInternal() && update_info->newRoomMemberBinAttrInternal()->size() != 0)
		{
			...
			u32 alloc = sce_update_info->newRoomMemberBinAttrInternalNum * sizeof(SceNpMatching2RoomMemberBinAttrInternal);
			sce_update_info->newRoomMemberBinAttrInternal = PSPPointer<SceNpMatching2RoomMemberBinAttrInternal>::Create(edata.Alloc(alloc));
