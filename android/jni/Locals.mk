...
# PPSSPP's curated mbedtls config. Global (not just the mbedtls module) so the
# library and everything that includes mbedtls headers agree on the SAME config
# - a mismatch changes struct sizes (silent ABI corruption). Matches the PUBLIC
# compile definition on the CMake side (ext/mbedtls-build).
# abspath: LOCAL_PATH may be relative (CI invokes ndk-build with a relative
# APP_BUILD_SCRIPT), and a relative path baked into the define won't resolve
# from the compiler's working directory.
LOCAL_CFLAGS += -DMBEDTLS_CONFIG_FILE=\"$(abspath $(LOCAL_PATH)/../../ext/mbedtls.h)\"
